...
with st.form(key="solar_prediction_form"):
    ...
    with col2:
        st.markdown("<b style='color:#E65100;'>☀️ 역학 및 에너지 인자</b>", unsafe_allow_html=True)
...
if submit_button:
    ...
    # ☀️ 탄소 감축량 체감 수치 스케일 업! (대형 발전단지 및 누적 가동 기준 환산 적용)
    # 기존 kW 단위를 대규모 전력망 기준(x1000)으로 확장하여 감축량 수치가 훨씬 크고 웅장하게 나오도록 변경했습니다.
    ...
    with res_col1:
        ...
        # ☀️ 수치가 묵직하게 찍히는 탄소 감축량 출력 구역
        st.caption(f"☀️ 대규모 단지 연계 시 온실가스 감축 효과: 약 {co2_reduction:.2f} kg CO₂eq")
...
st.caption("☀️ 모델: Gradient Boosting Regressor | 탄소 감축 근거: 공표된 2023년 전력배출계수 (0.4173 tCO2eq/MWh)")
