...
with main_col1:
    with st.form(key="solar_prediction_form"):
        ...
        # 슬라이더들을 한 줄에 두 개씩 배치하여 세로 길이를 압축
...
# --- 오른쪽 구역: 예측 결과 (버튼 클릭 전에는 안내 문구, 클릭 후에는 대형 결과 노출)
with main_col2:
    ...
    if submit_button:
        ...
        # 분석 요약 박스 (세로 폭을 위해 패딩과 글자 크기를 컴팩트하게 조절)
        st.markdown(f"""
